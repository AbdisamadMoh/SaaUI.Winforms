...
namespace SaaUI.Fonts
{
    /// <summary>
    /// Converts Awesome and Elegant Fonts to icons, bitmaps and images.
    /// For more information visit http://www.fontawesome.com  and  https://www.elegantthemes.com/blog/resources/elegant-icon-font
    /// </summary>
    internal static class FontImages
    {
        /// <summary>
        ...
        /// </summary>
        private const int MaxFontSize = 520;
        ...
        private static void CreateFonts()
        {
           ...
           string strFile = AppDomain.CurrentDomain.BaseDirectory;
            ...
            if (!Directory.Exists(Path.Combine(strDir, "IconFont")))
            {
             ...
             strFile = Path.Combine(strDir, "IconFont\\FontAwesome.ttf");
            if (!File.Exists(strFile))
            {
                var fs = System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("SaaUI.IconFont.FontAwesome.ttf");
            ...
            strFile = Path.Combine(strDir, "IconFont\\ElegantIcons.ttf");
            if (!File.Exists(strFile))
            {
                var fs = System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream("SaaUI.IconFont.ElegantIcons.ttf");
                FileStream sw = new FileStream(strFile, FileMode.Create, FileAccess.Write);
                fs.CopyTo(sw);
                sw.Close();
                fs.Close();
        ...
        static FontImages()
        {
          ...
          string  strFile = AppDomain.CurrentDomain.BaseDirectory;
            ...
            strFile = Path.Combine(strFile, "Fonts\\FontAwesome.ttf");
            ...
            get
            {
                if (m_fontsElegant.Count <= 0)
                {
                    lock (m_fontsElegant)
                    {
                        ...
                        {
                            string strFile = AppDomain.CurrentDomain.BaseDirectory;
                            strFile = Path.Combine(strFile, "Fonts\\ElegantIcons.ttf");
                            m_fontCollection.AddFontFile(strFile);
        ...
        /// <returns>ͼ��</returns>
        public static Icon GetIcon(Icons iconText, int imageSize = 32, Color? foreColor = null, Color? backColor = null)
        {
        ...
        /// <exception cref="FileNotFoundException">Font file not found</exception>
        public static Bitmap GetImage(Icons iconText, int imageSize = 32, Color? foreColor = null, Color? backColor = null)
        {
        ...
        private static Size GetIconSize(Icons iconText, Graphics graphics, Font font)
        {
